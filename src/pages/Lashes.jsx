...
export default function Lashes() {
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      ...
      const soloPestanas = datosTotales.filter(item => 
        item.categoria && item.categoria.toLowerCase() === 'pestañas'
      );
  ...
  if (isLoading) {
    return <div className="py-20 text-center text-gray-400 animate-pulse">Cargando estilos de pestañas...</div>;
  }

  return (
    <section id="pestañas" className="py-10 overflow-hidden">
      
      <div className="text-center mb-16 px-6">
        <p className="text-xs font-bold tracking-[0.2em] uppercase text-gray-400 mb-3">Nuestros Estilos</p>
        <h2 className="text-4xl font-serif font-bold text-gray-900">Que tus pestañas hablen por ti.</h2>
      </div>
